...
function Navbar({ hideButtons = false }) {
  ...
  return (
    <nav className="h-16 fixed top-0 left-0 right-0 z-10 w-full bg-pink-500 text-white shadow-lg">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex justify-between items-center h-full">
        ...
        <div className="flex items-center">
          ...
          >
            Minnu <span className="text-white">Skincare store</span>
          </h1>
        </div>

        {/* Desktop Menu */}
        <div className="hidden md:flex space-x-6 items-center">
          <button onClick={() => navigate('/')} className=" text-2xl font-semibold hover:text-pink-300 transition">
            Home
          </button>

          {/* Add Product Button (conditionally hidden) */}
          {!hideButtons && (
            <button onClick={() => navigate('/products')} className="bg-pink-500 px-4 py-2 rounded-lg hover:bg-pink-200 transition">
              Add Product
            </button>
          )}
          ...
          {user ? (
            <>
              <span className="text-2xl font-semibold">
                Hello,<span className="font-semibold text-white">{capitalize(user.name)}😊</span>
              </span>
